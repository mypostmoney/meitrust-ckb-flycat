...
function loginSuccess(signer: any) {
...
export async function getLoginInfo(request: LoginRequest): Promise<Signer> {
  ...
  switch (mode) {
    ...
    case LoginMode.nip07Wallet:
      ...
      //   throw new Error('window.nostr not found!');
      // }
      const pk = await request.ckbSigner?.originAddress;
      const isLoggedIn = pk != null && pk.length > 0;
      saveTempMyPublicKey(pk);
      ...
      return {
        ...
        publicKey: pk,
        getPublicKey: async () => {
          return await request.ckbSigner?.originAddress as string
        },
        signEvent: undefined,
        ckbSigner: request.ckbSigner
      };
