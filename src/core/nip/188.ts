...
function toFilterOptMode(num: number): FilterOptMode | undefined {
  const enumKeys = Object.keys(FilterOptMode).filter(
    key => !isNaN(Number(FilterOptMode[key as keyof typeof FilterOptMode])),
  );

  for (const key of enumKeys) {
    if (FilterOptMode[key as keyof typeof FilterOptMode] === num) {
      return FilterOptMode[key as keyof typeof FilterOptMode];
    }
  }
